...
const ASK_API_ROUTE = normalizeRoute(process.env.ASK_ASSISTANT_API_ROUTE || process.env.ASK_JOSHGPT_API_ROUTE || '/api/ask-assistant')
const LEGACY_ASK_API_ROUTE = '/api/ask-joshgpt'
const ASK_CONTEXT_DIR = process.env.ASK_ASSISTANT_CONTEXT_DIR || process.env.ASK_JOSHGPT_CONTEXT_DIR || '/app/context/docs'
const OPENAI_API_KEY = process.env.OPENAI_API_KEY || ''
const OPENAI_BASE_URL = (process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/$/, '')
const ASK_ASSISTANT_MODEL = process.env.ASK_ASSISTANT_MODEL || process.env.ASK_JOSHGPT_MODEL || 'gpt-4o-mini'
const ASK_ASSISTANT_MAX_TOKENS = parseInteger(process.env.ASK_ASSISTANT_MAX_TOKENS || process.env.ASK_JOSHGPT_MAX_TOKENS, 700, 200, 1600)
const ASK_ASSISTANT_TEMPERATURE = parseNumber(process.env.ASK_ASSISTANT_TEMPERATURE || process.env.ASK_JOSHGPT_TEMPERATURE, 0.2, 0, 1)
const ASK_ASSISTANT_TIMEOUT_MS = parseInteger(process.env.ASK_ASSISTANT_TIMEOUT_MS || process.env.ASK_JOSHGPT_TIMEOUT_MS, 30000, 5000, 120000)
const ASK_ASSISTANT_RATE_LIMIT_WINDOW_SECONDS = parseInteger(process.env.ASK_ASSISTANT_RATE_LIMIT_WINDOW_SECONDS || process.env.ASK_JOSHGPT_RATE_LIMIT_WINDOW_SECONDS, 300, 30, 3600)
const ASK_ASSISTANT_RATE_LIMIT_MAX = parseInteger(process.env.ASK_ASSISTANT_RATE_LIMIT_MAX || process.env.ASK_JOSHGPT_RATE_LIMIT_MAX, 10, 1, 200)
const ASK_ASSISTANT_MAX_QUESTION_CHARS = parseInteger(process.env.ASK_ASSISTANT_MAX_QUESTION_CHARS || process.env.ASK_JOSHGPT_MAX_QUESTION_CHARS, 1200, 100, 5000)

const ASK_RATE_LIMIT_WINDOW_MS = ASK_ASSISTANT_RATE_LIMIT_WINDOW_SECONDS * 1000
...
const ASK_CONTEXT_SECTIONS = loadAskAssistantContext()
...
function loadAskAssistantContext() {
  const definitions = [
...
function isRateLimited(ipAddress) {
  ...
  if (current.count >= ASK_ASSISTANT_RATE_LIMIT_MAX) {
    return true
  }
...
function servePrivateResume(req, res, requestUrl) {
  ...
  if (Number.isNaN(expiresAt) || expiresAt <= now) {
    sendText(req, res, 410, 'Link has expired. Contact the site owner for a new link.')
    return
  }
...
async function askOpenAI(question, sections) {
  ...
  const systemPrompt = [
    'You are a site assistant. You are searching this website and public GitHub project context.',
    'Use only the provided context sections.',
    'Do not invent facts that are not present in the supplied context.',
    'If the answer is not in context, clearly say so and suggest contacting the site owner.',
  ...
  const timeout = setTimeout(() => controller.abort(), ASK_ASSISTANT_TIMEOUT_MS)
  ...
  try {
    response = await fetch(`${OPENAI_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        model: ASK_ASSISTANT_MODEL,
        temperature: ASK_ASSISTANT_TEMPERATURE,
        max_tokens: ASK_ASSISTANT_MAX_TOKENS,
        messages: [
...
async function handleAskAssistant(req, res) {
  if (req.method !== 'POST') {
  ...
  if (!OPENAI_API_KEY) {
    sendJson(req, res, 503, { error: 'Ask Assistant is not configured on this server yet.' })
    return
  }
  ...
  if (question.length > ASK_ASSISTANT_MAX_QUESTION_CHARS) {
    sendJson(req, res, 400, { error: `Question exceeds ${ASK_ASSISTANT_MAX_QUESTION_CHARS} characters.` })
    return
  }

  const contextSections = pickContextSections(question)
  if (contextSections.length === 0) {
    sendJson(req, res, 500, { error: 'No local context available for Ask Assistant.' })
    return
  }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unexpected Ask Assistant error'
...
const server = createServer(async (req, res) => {
  ...
  if (isRouteMatch(ASK_API_ROUTE, requestUrl.pathname) || isRouteMatch(LEGACY_ASK_API_ROUTE, requestUrl.pathname)) {
    await handleAskAssistant(req, res)
    return
  }
...
server.listen(PORT, () => {
  ...
  console.log(`Ask Assistant API route: ${ASK_API_ROUTE}`)
